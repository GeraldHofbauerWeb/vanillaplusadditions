...
- Event priority is part of feature behavior: `HauntedHouseModule` uses `HIGHEST`/`HIGH` on `FinalizeSpawnEvent`; `overpacked_extensions`' `SlowdownFeature` uses `LOW` to run after Overpacked’s default handler.
