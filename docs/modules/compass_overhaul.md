...
that carries the whole icon, so not a pixel of it is touched. The PNGs are therefore derived Mojang
assets.
...
and the generator aborts if one does, so the casing is provably identical everywhere. Five roles
are recoloured:
