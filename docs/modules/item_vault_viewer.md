...
not sneaking, the target really a vault, and a 64-block range. The held modifier and the hand are
the two things it cannot see, so those stay client-side. The spectator check stays client-side too,
although the server could just as well make it.
...
`KeyMapping.isDown()` does track the held state, but it is bookkeeping the client itself resets —
opening any screen calls `KeyMapping.releaseAll()` — and NeoForge folds the conflict context and the
key modifier into the same answer. The raw read asks "is this key physically down right now" and
nothing else. Bypassing the mapping is also why the `Type.MOUSE` branch has to exist:
`InputConstants.isKeyDown` is `glfwGetKey` and only covers keyboard keys, so a modifier rebound to a
mouse button has to be polled through `glfwGetMouseButton`.
...
The member order mirrors `ItemVaultBlockEntity.initCapability`, which knows exactly two orders: a
`Z`-axis vault walks z, then x, then y, everything else x, then y, then z. That keeps the slot order
identical to what the placed vault's combined handler would produce. `ContraptionVaultAccess` also
carries a `Y` case, but a vault's axis is read from the `HORIZONTAL_AXIS` property, so that branch
...
instead of the vault you clicked. It only runs when not one of the resolved member positions has a
mounted storage — the two controller lookups failing is not enough on its own, because the clicked
block then becomes its own controller and its own storage is normally read directly.
...
| | |
|---|---|
| Grid | 9 columns; `ceil(distinct types / 9)` rows, of which at most 6 are visible. The window height is fixed when the menu opens, so filtering never shrinks the panel — a vault that opened six rows tall stays six rows tall with three items left in it |
...
Under its wrappers the vault's handler is a plain `ItemStackHandler` and reports that class's
default slot limit, `Item.ABSOLUTE_MAX_STACK_SIZE` = 99 — never below an item's own stack size, so
`limit` is in practice the item's maximum stack size. A slot with 32 cobblestone therefore counts as
half full and a slot with a single shulker box as completely full — closer to "how much still fits
in there" than a slot count is. The slot count is reported separately and lives in the bar's hover
tooltip, together with the total number of items.
